...
## 2025-05-21 - Short-Circuiting Dictionary Sorting & Deferring F-String Evaluation
**Learning:** Calling `dict(sorted(d.items()))` on empty or single-element mappings creates unnecessary iterator allocations, sorting, and dictionary reconstruction. Checking `len(d) <= 1` and returning `dict(d)` directly provides a ~3x speedup while preserving key-order guarantees. Furthermore, evaluating f-strings eagerly on helper parameters (e.g. `f"{socket}.location"`) creates heap string allocations on happy-path execution; passing format components separately and deferring formatting until exception raising eliminates unnecessary string creation.
**Action:** Short-circuit `sorted(d.items())` with `dict(d)` when `len(d) <= 1`, and defer f-string formatting in hot helper functions until exception paths.
