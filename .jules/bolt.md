...
## 2025-05-21 - Deferring String Formatting & Exact Type Fast-Pathing in Vector Resolvers
**Learning:** In hot transformation functions like `_vec3` and `_transform_from_scene` in `resolver.py`, eager string interpolation (`f"{socket}.{field}"`) creates redundant string heap allocations on every happy-path execution. Accepting raw socket/field strings and constructing formatted error messages only when raising exceptions completely avoids formatting overhead. In addition, using exact type checks (`type(value) is list`) fast-paths standard JSON vectors, and extracting `scene_manifest["sockets"]` once outside the step iteration eliminates dictionary lookups. Together, these clean optimizations improved `resolve_build` performance by ~13.3% (from 10.75s to 9.32s for 200,000 runs).
**Action:** Defer f-string interpolation until exception handling blocks in hot helper functions and hoist dict lookups out of step iteration loops.
