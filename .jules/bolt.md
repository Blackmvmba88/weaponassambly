...
## 2025-02-14 - Eliminating Redundant Validation Passes and Lambda Sort Keys in Pipeline Hot Paths
**Learning:** Calling validation functions multiple times across composed pipeline functions (e.g. calling `validate_build` directly in `resolve_build` right before `plan_build`, which internally calls `validate_build`) duplicates validation effort. Relying on downstream pipeline functions to perform validation avoids duplicate checks. Furthermore, sorting a list of tuples `(stage, slot, module)` with `key=lambda item: (item[0], item[1], item[2])` adds unnecessary Python function call overhead, whereas native `list.sort()` compares tuple elements directly in C.
**Action:** Avoid redundant validation calls when delegating to subroutines that already perform validation, and use native `list.sort()` when tuple elements are already ordered by the desired sort key.
