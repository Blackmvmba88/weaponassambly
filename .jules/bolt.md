...
## 2025-02-15 - Precomputing Enum Representation Lookups & Direct Tuple Sorting
**Learning:** In hot serialization and build planning paths, calling `.name.lower()` repeatedly on Enum members (such as `AssemblyStage`) incurs dynamic attribute lookup and string allocation overhead on every iteration (~1.7x slower). Precomputing a lookup dictionary `STAGE_NAMES = {stage: stage.name.lower() for stage in AssemblyStage}` avoids runtime string lowercasing entirely. Furthermore, using `pending.sort()` directly instead of `pending.sort(key=lambda item: (item[0], item[1], item[2]))` leverages C-level elementwise tuple comparisons, avoiding lambda frame allocation and redundant key tuple creation.
**Action:** Precompute static Enum string transformations in a module-level constant map for hot serialization paths, and avoid redundant key lambdas when sorting tuples whose natural ordering matches the target sort key.
