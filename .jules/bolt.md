...
## 2025-02-15 - Fast Exact Type Checks and Inlined Validation in Scene Manifest Hot Path
**Learning:** In hot validation routines processing deserialized JSON structures, `isinstance()` checks incur overhead due to MRO class hierarchy traversal. Replacing `isinstance(c, (int, float))` with exact type checking (`type(c) is float or type(c) is int`) provides a pure C-level pointer comparison speedup while correctly distinguishing booleans (`type(True)` is `bool`). Additionally, inlining field checks (such as unit scale validation) within primary iteration loops avoids redundant dictionary lookups (`transform.get("scale")`) and secondary component loops, yielding a 2.21x speedup in `validate_scene_manifest`.
**Action:** Prefer exact type checks (`type(v) is float or type(v) is int`) for fixed vector component verification in hot JSON validation loops, and inline dependent field validations inside the primary field loop.
