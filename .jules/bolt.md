...
## 2025-05-21 - Fast Type Checks and Field Loop Consolidation
**Learning:** In CPython, `isinstance(x, (int, float)) and not isinstance(x, bool)` incurs overhead due to class inheritance hierarchy traversal and double checks. Using exact type checks `type(x) is float or type(x) is int` eliminates hierarchy traversal while automatically excluding `bool` (`type(True)` is `bool`). Consolidating field validation and scale checks inside a single loop avoids redundant dict lookups and list re-validations, yielding a ~1.8x overall speedup in `validate_scene_manifest` while maintaining clean loop structures.
**Action:** Use exact type checks (`type(x) is float or type(x) is int`) in hot validation loops for fixed scalar types and consolidate field-dependent assertions into primary iteration loops.
