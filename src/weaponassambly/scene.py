...
def validate_scene_manifest(data: dict[str, Any]) -> SceneValidationResult:
    ...
    for socket_name, transform in sockets.items():
        ...
        for field in TRANSFORM_FIELDS:
            ...
            # Fast-path exact type checks (float or int) avoiding class hierarchy traversal
            has_non_number = False
            for component in value:
                if not (type(component) is float or type(component) is int):
                    has_non_number = True
                    break
            if has_non_number:
                errors.append(f"socket {socket_name}.{field} must contain only numbers")
            elif field == "scale":
                # Inlined scale check to eliminate redundant dict lookups and re-validation
