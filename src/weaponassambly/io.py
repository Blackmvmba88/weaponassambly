...
def load_build(path: str | Path) -> BuildConfig:
    data = load_json(path)
    # Fast-path: check key existence using direct boolean operations to avoid
    # constructing a list comprehension and temporary list on valid input loads (~1.76x speedup).
    if not (
